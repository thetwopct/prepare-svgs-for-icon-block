...
export function processSvgs(options) {
	const { directory: svgDir, outputFile = 'output.js', addExport = false, framedIcons, forcePathColor, fillColor } = options;
	const output = [];
	...
	const outPath = path.join(svgDir, outputFile);
  ...
  .join(',\n');
	...
	const exportStatement = addExport ? '\nexport default customIcons;\n' : '';
	fs.writeFileSync(
		outPath,
		`const customIcons = [\n${outputJs}\n];\n${exportStatement}`
	);
